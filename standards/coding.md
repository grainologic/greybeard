...
- **Report.** Code first, then at most three short lines: what changed, what was skipped, when to add it (`did X; skipped Y, add when Z`). Y is never a never-cut item. If the explanation outweighs the diff, cut the explanation.
...
- **Comment every non-obvious ladder decision.** Obvious choices stay silent. For a new dependency, a reuse over a rewrite, or a corner cut: name what you chose and what you rejected, and for the cut, the ceiling and the upgrade path. {{marker_line}}
